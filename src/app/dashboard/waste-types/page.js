...
import { formatIntegerInputID, formatNumberID, parseNumberID } from '@/utils/numberFormat';
...
export default function WasteTypesPage() {
  ...
  const handleEdit = (item) => {
    ...
    setFormData({
      ...item,
      purchase_price: formatIntegerInputID(item.purchase_price),
      sales_price: formatIntegerInputID(item.sales_price),
    })
  ...
  const handleAdd = () => {
    setIsEditing(false)
    setFormData({ id: null, name: '', uom_id: '', purchase_price: '', sales_price: '' })
    document.getElementById('modal_waste').showModal()
  }
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEditing) {
      const { error: err } = await supabase.from('waste_types').update({
        ...
        purchase_price: parseNumberID(formData.purchase_price),
        sales_price: parseNumberID(formData.sales_price)
      }).eq('id', formData.id)
      error = err
    } else {
      const { id, ...payload } = formData
      const { error: err } = await supabase.from('waste_types').insert({
        ...payload,
        purchase_price: parseNumberID(formData.purchase_price),
        sales_price: parseNumberID(formData.sales_price),
      })
          ...
          <tbody>
            {filteredWaste.map((w) => (
              <tr key={w.id}>
                ...
                <td className='text-right'>{w.purchase_price.toLocaleString('id-ID')}</td>
                <td className='text-right'>{w.sales_price.toLocaleString('id-ID')}</td>
                <td className='text-right'>
                  <span className={`badge ${w.current_stock > 0 ? 'badge-success' : 'badge-warning'} text-white`}>
                    {formatNumberID(w.current_stock)}
                  </span>
                </td>
              ...
              <div className="form-control">
                ...
                <input
                  ...
                  onChange={e => setFormData({
                    ...formData,
                    purchase_price: formatIntegerInputID(e.target.value),
                  })}
                />
              </div>
              <div className="form-control">
                ...
                <input
                  ...
                  onChange={e => setFormData({
                    ...formData,
                    sales_price: formatIntegerInputID(e.target.value),
                  })}
                />
              </div>
            </div>
