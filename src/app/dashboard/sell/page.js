...
import { formatNumberID, normalizeDecimalInputID, parseNumberID } from '@/utils/numberFormat';
...
export default function SellPage() {
  ...
  const addToBatch = () => {
    const qty = parseNumberID(tempQty)
    if (!tempItem || qty <= 0) return

    const waste = wasteTypes.find(w => w.id == tempItem)

    if (qty > waste.current_stock) {
      alert(`Stok tidak cukup! Sisa stok: ${formatNumberID(waste.current_stock)}`)
      return
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <select className="select select-bordered" value={tempItem} onChange={e => {
                ...
                {wasteTypes.map(w => (
                  <option key={w.id} value={w.id}>
                    {w.name} ({formatNumberID(w.current_stock)} {w.uoms.name}) - @ Rp {formatNumberID(w.sales_price)}
                  </option>
                ))}
              </select>
            </div>
            <div className="form-control mb-4">
              <label className="label">Jumlah ({tempUOM})</label>
              <input
                type="text"
                inputMode="decimal"
                ...
                value={tempQty}
                onChange={e => {
                  const input = normalizeDecimalInputID(e.target.value)
                  const maxQty = wasteTypes.find(w => w.id == tempItem)?.current_stock || 0
                  const qty = parseNumberID(input)

                  setTempQty(qty > maxQty ? formatNumberID(maxQty) : input)
                }}
              />
            </div>
            <button className="btn btn-info text-white w-full" onClick={addToBatch} disabled={!tempItem || parseNumberID(tempQty) <= 0}>+ Tambah ke Keranjang</button>
          </div>}
        ...
        <div className="card bg-white shadow h-fit">
                ...
                <tbody>
                  {cart.map((item, idx) => (
                    <tr key={idx}>
                      ...
                      <td className='text-center'>{formatNumberID(item.qty)}</td>
                      <td className='text-center'>{item.uom}</td>
                      <td className='text-right truncate'> {item.subtotal.toLocaleString('id-ID')}</td>
          ...
          <div className='flex gap-1 m-4 ms-5'>
            ...
            <p className="text-lg text-primary font-semibold">Rp {grandTotal.toLocaleString('id-ID')}</p>
          </div>
