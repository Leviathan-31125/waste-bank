...
export default function DepositPage() {
    ...
    useEffect(() => {
        const fetchMasters = async () => {
            ...
            const { data: listBatch } = await supabase
    ...
    const addToBatch = () => {
        ...
        const subtotal = waste.purchase_price * qty
                    ...
                    <div className="flex lg:flex-row md:flex-row sm:flex-row flex-col lg:gap-3 gap-2 lg:items-end md:items-end sm:items-end ">
                        <div className="form-control w-full">
                            ...
                            >
                                ...
                                {wasteTypes.map(w => (
                                    <option key={w.id} value={w.id}>{w.name} (@ Rp {formatNumberID(w.purchase_price)})</option>
                                ))}
                            </select>
                        </div>
                        <div className="form-control lg:w-32 md:w-32 w-full">
                            ...
                            <input
                                ...
                                value={tempQty}
                                onChange={e => setTempQty(normalizeDecimalInputID(e.target.value))}
                            />
                        </div>
                        <button className="btn btn-info text-white" onClick={addToBatch} disabled={!tempItem || parseNumberID(tempQty) <= 0}>+ Tambah</button>
                    </div>
                </div>
            </div>
                        ...
                        <tbody>
                            {cart.map((item, idx) => (
                                <tr key={idx}>
                                    ...
                                    <td className='text-right'>Rp {formatNumberID(item.price)}</td>
                                    <td className='text-center'>{formatNumberID(item.qty)}</td>
                                    <td className='text-right truncate'>Rp {item.subtotal.toLocaleString('id-ID')}</td>
