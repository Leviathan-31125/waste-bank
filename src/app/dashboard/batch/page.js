        ...
        <table className="table">
          <thead>
            <tr>
              ...
              <th className='text-center'>Qty Transaksi</th>
          ...
          <tbody>
            {filteredBatch.map((b) => (
              <tr key={b.id} className={b.status === 'non-active' ? 'bg-gray-100 text-gray-500' : ''}>
                ...
                <td className="font-mono font-bold text-success text-center truncate">{b.transaction_count.toLocaleString()}</td>
                <td className="font-mono font-bold text-success text-right truncate">Rp {b.total_value.toLocaleString()}</td>
                <td>
